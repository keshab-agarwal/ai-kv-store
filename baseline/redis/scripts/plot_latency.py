#!/usr/bin/env python3
"""
Plot YCSB latency distributions from CSV output (Redis benchmark variant).

Usage:
    python3 plot_latency.py <latencies.csv> [output_dir]

Output files:
    latency_histogram.png — per-operation histograms with p50/p95/p99 markers
    latency_cdf.png       — per-operation cumulative distribution functions
"""
import sys
import os
import csv
...
def main():
    ...
    with open(csv_path) as f:
        ...
        for row in reader:
            ...
            latencies[op].append(us / 1000.0)

    if not latencies:
        print("No latency data found in CSV.", file=sys.stderr)
        return
    ...
    for idx, (op, lats) in enumerate(sorted(latencies.items())):
        ...
        ax.hist(lats_arr, bins=100, alpha=0.7, color="#e74c3c", edgecolor="black", linewidth=0.3)
        ax.axvline(p50, color="green", linestyle="--", label=f"p50={p50:.2f}ms")
        ax.axvline(p95, color="orange", linestyle="--", label=f"p95={p95:.2f}ms")
        ax.axvline(p99, color="darkred", linestyle="--", label=f"p99={p99:.2f}ms")
        ax.set_title(f"Redis {op} Latency (n={len(lats)})")
        ax.set_xlabel("Latency (ms)")
        ax.set_ylabel("Count")
        ax.legend(fontsize=8)

    plt.tight_layout()
    hist_path = os.path.join(out_dir, "latency_histogram.png")
    ...
    for idx, (op, lats) in enumerate(sorted(latencies.items())):
        ...
        ax.plot(lats_sorted, cdf, color="#e74c3c", linewidth=1.5)
        ax.axhline(0.95, color="orange", linestyle=":", alpha=0.7, label="p95")
        ax.axhline(0.99, color="darkred", linestyle=":", alpha=0.7, label="p99")
        ax.set_title(f"Redis {op} Latency CDF")
        ax.set_xlabel("Latency (ms)")
